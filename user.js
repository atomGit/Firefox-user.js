...
user_pref("browser.display.use_document_fonts", 1);             // '0' breaks the display of uBlock and uMatrix
...
user_pref("dom.indexedDB.enabled", true);                       // as ghacks said, disabling this will break some sites and extensions
...
user_pref("extensions.systemAddon.update.url", "");             // disallow updates for system add-ons (Firefox Features) that ship with the browser
...
user_pref("webgl.disabled", false);                             // `true` will break some sites including Google Earth view on Google Maps - WenGL appears to be dependant upon JS being enabled (which should be disabled globally by default) so if you trust a site enough to enable JS, you may as well enable WebGL as well since the security risks with WebGL appear to be far less of a problem than JS
user_pref("webgl.disable-extensions", false);                   // 'true' breaks Google Earth view on Google Maps
user_pref("webgl.min_capability_mode", false);                  // 'true' breaks Google Earth view on Google Maps
...
/*
 * === 12BYTES.ORG SEARCH ENGINE SANITIZE ===
*/
...
user_pref("browser.fullscreen.autohide", false);                 // whether to hide tool bars and tabs when entering full-screen view (F11)
